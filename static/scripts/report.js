...
reportResultsApp.controller('SpecialistCtrl', function ($scope, $http, $interval) {
    $scope.host = '';
    $scope.cookbook = '';
    $scope.limitList = [10, 25, 50];
    $scope.limit = $scope.limitList[1];
    $scope.apiResponse = [];

    $scope.sendApiRequest = function() {
        var url = '/api/reports?hostname=' + $scope.host
            + '&cookbook_name=' + $scope.cookbook
            + '&limit=' + $scope.limit;
        $http.get(url).
            ...
            error(function(data, status, headers, config) {
                console.error('We had an error', status, data);
                $scope.apiResponse = [];
            });
    };
    $scope.sendApiRequest();
    $interval($scope.sendApiRequest, 30000);
...
reportResultsApp.controller("reportResultCtrl", [ "$scope", "$modal", "$location", "data", "filename", function($scope, $modal, $location, data, filename) {
    if (data === null) {
        ...
        $scope.results = formatResults(data.results);
...
var formatResults = function(input) {
    ...
    var resultRoles = function(r) {
        console.log(r)
        return _.uniq(_.map(r, exampleRole),
    ...
    _.each(roleSets, function(rs) {
        ...
        var tests = _.map(input, function(r) {
            ...
            var examples = _.filter(r, function(e) {
